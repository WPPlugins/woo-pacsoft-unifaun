...
class Request {
	...
	public static function generateXML( $orderId, $serviceId = '' )
	{
		...
		$name = $order->shipping_first_name ? "{$order->shipping_first_name} {$order->shipping_last_name}" : "{$order->billing_first_name} {$order->billing_last_name}";
		...
		$zipcode = $order->shipping_postcode ? $order->shipping_postcode : $order->billing_postcode;
		...
		$val = $receiver->addChild( 'val', $order->billing_email );	
		$val->addAttribute( 'n', "email" );
		
		$val = $receiver->addChild( 'val', $order->billing_phone );
		...
		if( get_option( 'pacsoft_addon_enot' ) ) {
			...
			$val = $option->addChild( 'val', $order->billing_email );
			$val->addAttribute( 'n', "to" );
		...
		if( get_option( 'pacsoft_addon_sms' ) ) {
			...
			$val = $addon->addChild( 'val', $order->billing_phone );
			...
			if( "IT16" == $serviceId ) {
				...
				$val = $addon->addChild( 'val', $order->billing_email );
				$val->addAttribute( 'n', "misc" );
			}
		}
